...
# fetch-and-run-installer-pkg.py was created to silently automate the installation 
# process and add optional arguments to immediately run `startosinstall` 
# as either an in-place installation or perform an erase and install 
...
'''fetch-and-run-installer-pkg.py
A tool to download the a pkg installer for the Install macOS app from Apple's
softwareupdate servers and automatically install it to a system'''
...
def main():
    ...
    parser.add_argument('--upgradesilent', action='store_true',
                        help='Automatically start in-place upgrade to latest macOS version with no user interaction.')
    parser.add_argument('--eraseinstall', action='store_true',
                        help='Automatically performs an erase and install to latest macOS version with no user interaction. HANDLE WITH CARE!')
    ...
    # automatically install pkg file to /Applications
    install_cmd = ['installer', '-pkg', local_pkg, '-target', '/']
    subprocess.check_call(install_cmd)
    startosinstall = '"/Applications/Install macOS Big Sur.app/Contents/Resources/startosinstall"'
    verify_OS_app = [startosinstall, '--usage']
    subprocess.check_call(verify_OS_app)
    time.sleep(120)
    
    # run os installer cmds
    run_installer_cmd = [startosinstall, '--agreetolicense', '--forcequitapps']
    
    if args.upgradesilent:
        subprocess.check_call(run_installer_cmd)
    elif args.eraseinstall:
        e_i_cmd_args = ['--eraseinstall', '--newvolumename', '"Macintosh HD"']
        e_and_i_cmd = run_installer_cmd.extend(e_i_cmd_args)
        subprocess.check_call(e_and_i_cmd)
